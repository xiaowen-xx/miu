...
if (msgInputArea) {
    ...
    msgInputArea.addEventListener('focus', function() {
        ...
        if (chatRoom) chatRoom.classList.add('input-focused');
    ...
    // ★★★ 新增：输入框失去焦点时解锁 ★★★
    msgInputArea.addEventListener('blur', function() {
        const chatRoom = document.getElementById('chatRoom');
        if (chatRoom) {
            setTimeout(() => {
                chatRoom.classList.remove('input-focused');
            }, 100);
        }
    });
...
// iOS 键盘适配精简版 - 替换 script.js 最后的匿名函数
(function() {
    // 简单的环境检测
...
(function() {
    ...
    const chatRoom = document.getElementById('chatRoom');
    const msgInput = document.getElementById('msgInput');
    const msgsArea = document.getElementById('roomMessages');

    // 核心逻辑：利用 VisualViewport API 实时锁定高度
    if (window.visualViewport) {
        ...
        const resizeHandler = () => {
            ...
            // 2. ★ 强行修改 DOM 高度，不用动画，直接瞬移
            // 这样输入框 (absolute bottom:0) 就会瞬间吸附在键盘上方
            chatRoom.style.height = vh + 'px';
        ...
        // 失去焦点时（收起键盘），恢复 100% 高度
        if (msgInput) {
            msgInput.addEventListener('blur', () => {
                // 延时一下，防止点击发送按钮时闪烁
                setTimeout(() => {
                    chatRoom.style.height = '100%';
                    window.scrollTo(0, 0); // 归位
                }, 100);
            });
        }
    }
